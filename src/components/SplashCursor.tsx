...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    let webglCtx: ReturnType<typeof getWebGLContext> | null = null;
    try {
      webglCtx = getWebGLContext(canvas);
    } catch (e) {
      console.warn('WebGL not supported for SplashCursor:', e);
      return;
    }
    ...
    function generateColor() {
      if (usePrimaryColors) {
        // Soft, eye-pleasing muted navy & subtle deep teal palette (low intensity, executive & comfortable for eyes)
        const primaryColors = [
          { r: 0x17 / 255, g: 0x2a / 255, b: 0x45 / 255 }, // Soft Navy Slate (#172a45)
          { r: 0x20 / 255, g: 0x3a / 255, b: 0x5e / 255 }, // Dark Ambient Indigo (#203a5e)
          { r: 0x1e / 255, g: 0x4e / 255, b: 0x58 / 255 }, // Muted Deep Teal (#1e4e58)
          { r: 0x11 / 255, g: 0x5e / 255, b: 0x59 / 255 }, // Soft Deep Emerald (#115e59)
          { r: 0x0f / 255, g: 0x3d / 255, b: 0x4a / 255 }, // Dark Cyan Shadow (#0f3d4a)
        ];
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getEventPosition(touches[i]);
        const posX = scaleByPixelRatio(pos.x);
        const posY = scaleByPixelRatio(pos.y);
  ...
  return (
    <div
      ref={containerRef}
      className={clsx(
        children ? 'relative overflow-hidden' : 'fixed inset-0 pointer-events-none z-0',
        containerClassName,
      )}
    >
      <canvas
      ...
      />
      {children && <div className={clsx('relative z-10', className)}>{children}</div>}
    </div>
  );
}
